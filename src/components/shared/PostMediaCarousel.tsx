...
export function PostMediaCarousel({ media, aspect = "4 / 5", onRemove }: {
  media: CarouselMedia[]; aspect?: string; onRemove?: (id: string) => void;
}) {
  ...
  const goTo = (i: number) => {
    ...
    el.scrollTo({ left: el.clientWidth * c, behavior: "smooth" }); setIdx(c);
  };
  ...
  return (
    <div className="relative w-full bg-black overflow-hidden" style={{ aspectRatio: aspect }}>
          ...
          <div key={m.id ?? i} className="relative w-full h-full shrink-0 snap-center">
            ...
            {onRemove && m.id && (
              <button type="button" onClick={() => onRemove(m.id!)}
                className="absolute top-2 left-2 bg-black/60 text-white rounded-full p-1.5 hover:bg-black/80"><X className="h-4 w-4" /></button>
            )}
          </div>
        ))}
      </div>
      {n > 1 && (
        <>
          {idx > 0 && <button type="button" onClick={() => goTo(idx - 1)} className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 text-white flex items-center justify-center hover:bg-black/60"><ChevronLeft className="h-5 w-5" /></button>}
          {idx < n - 1 && <button type="button" onClick={() => goTo(idx + 1)} className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 text-white flex items-center justify-center hover:bg-black/60"><ChevronRight className="h-5 w-5" /></button>}
          <span className="absolute top-3 right-3 bg-black/55 text-white text-[11px] font-bold px-2.5 py-0.5 rounded-full">{idx + 1}/{n}</span>
