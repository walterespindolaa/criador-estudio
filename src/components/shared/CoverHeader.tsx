...
export function CoverHeader({ label, title, count, from, to, ink = "#fff", sub = "rgba(255,255,255,.78)", hint }:{ label?:string; title:string; count?:number; from:string; to:string; ink?:string; sub?:string; hint?:string }) {
  return (
    <div className="relative overflow-hidden rounded-[18px] px-5 pt-7 pb-8 shadow-warm-lg"
      ...
      <div className="relative mt-3 flex items-end gap-1" style={{ ["--ch-ink" as string]: ink }}>
        <h2 className="font-display italic font-light text-[2.4rem] leading-[0.96] tracking-tight"
