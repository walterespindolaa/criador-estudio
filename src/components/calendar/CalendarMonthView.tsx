...
function buildMonthCells(currentMonth: Date): Cell[] {
  ...
  while (cells.length > 35 && cells.slice(35).every((c) => c.isOtherMonth)) {
    cells.length = 35;
  }
  return cells;
}
    ...
    <div className="bg-card rounded-xl border border-border overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-border">
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={() => onMonthChange(-1)} aria-label="Mês anterior">
            <ChevronLeft className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="sm" onClick={() => onMonthChange(1)} aria-label="Próximo mês">
            <ChevronRight className="h-4 w-4" />
          </Button>
          {!isCurrentMonth && (
            <Button variant="ghost" size="sm" onClick={() => onMonthChange(0)}>
              Hoje
            </Button>
          )}
        </div>
      ...
      <div className="grid grid-cols-7 border-b border-border bg-muted/20">
        {WEEK_DAY_LABELS.map((label) => (
          <div
            key={label}
            className="text-center py-2 text-[11px] uppercase tracking-wider font-body font-semibold text-muted-foreground"
          >
            {label}
          </div>
        ))}
        ...
        {cells.map((cell) => {
          ...
          return (
            ...
            >
              <p
                className={cn(
                  "text-xs font-body font-semibold mb-1",
              ...
              {dayPosts.slice(0, 3).map((post) => {
                ...
                return (
                  <button
                    ...
                    style={pillar ? { borderLeftColor: pillar.color, borderLeftWidth: 2 } : undefined}
                    className={cn(
                  ...
                  >
                    {post.scheduled_time && (
                      <span className="font-semibold mr-1">{post.scheduled_time.slice(0, 5)}</span>
                    )}
                    {post.title}
                  </button>
                );
              ...
              {dayPosts.length > 3 && (
                <button
                  ...
                  className="text-[10px] text-primary font-body font-medium hover:underline"
                >
