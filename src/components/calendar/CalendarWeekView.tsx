    ...
    <div className="bg-card rounded-xl border border-border overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-border">
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={() => onWeekChange(-1)} aria-label="Semana anterior">
            <ChevronLeft className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="sm" onClick={() => onWeekChange(1)} aria-label="Próxima semana">
            <ChevronRight className="h-4 w-4" />
          </Button>
          {weekOffset !== 0 && (
            <Button variant="ghost" size="sm" onClick={() => onWeekChange(-weekOffset)}>
              Esta semana
            </Button>
          )}
        </div>
      ...
      <div className="grid grid-cols-[60px_repeat(7,1fr)] border-b border-border bg-card sticky top-0 z-10">
        ...
        {weekDays.map((day) => {
          ...
          return (
            ...
            >
              <p className="text-[11px] uppercase tracking-wider font-body font-semibold text-muted-foreground">
                {day.name}
              </p>
              <p
                className={cn(
      ...
      <div className="grid grid-cols-[60px_repeat(7,1fr)] border-b border-border bg-muted/20">
        <div className="px-2 py-2 text-[10px] font-body font-semibold uppercase text-muted-foreground flex items-center justify-end gap-1">
          <Clock className="h-3 w-3" /> Sem horário
        </div>
        {weekDays.map((day) => {
          ...
          return (
            <div
              ...
              className={cn(
                "border-l border-border p-1 space-y-1 min-h-[50px]",
                day.date === today && "bg-primary/[0.03]"
              )}
            >
              {dayPosts.map((post) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "w-full text-left rounded-md border px-1.5 py-1 hover:shadow-warm-md hover:scale-[1.01] transition-all duration-150",
      ...
      <div className="relative grid grid-cols-[60px_repeat(7,1fr)]" style={{ height: HOURS.length * HOUR_HEIGHT }}>
        <div className="relative">
          {HOURS.map((h) => (
            <div
              key={h}
              className="absolute left-0 right-0 text-[10px] font-body font-semibold uppercase text-muted-foreground text-right pr-2"
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      borderLeftWidth: pillar ? 3 : 1,
                    }}
                    className={cn(
                      "absolute left-1 right-1 rounded-lg border p-2 cursor-pointer overflow-hidden",
                      "hover:shadow-warm-md hover:scale-[1.01] transition-all duration-150 group",
                      getStatusClasses(post.status)
                    )}
                  >
                    <span className="text-[10px] font-body font-semibold opacity-80">
                      {post.scheduled_time?.slice(0, 5)}
                    </span>
                    <p className="text-xs font-body font-medium line-clamp-1 mt-0.5">{post.title}</p>
                    {post.format && (
                      <span className="text-[9px] font-body opacity-60 capitalize">{post.format}</span>
                    )}
                  </button>
                );
              })}
