...
export type ExternalPost = {
  ...
  /* Observação da social mídia pro card. É o que o parceiro lê como
     "observações da social mídia" no card dele; sem campo no formulário ela
     não tinha onde escrever (Gabriela, 09/09/2026). */
  notes: string | null;
...
export type ExternalPostInput = { title: string; platform: string; format: string; caption?: string | null; hook?: string | null; script?: string | null; notes?: string | null; approval_mode?: "fast" | "flow" | "both"; scheduled_date?: string | null; scheduled_time?: string | null; reference_url?: string | null; drive_folder_url?: string | null; editorial_line_id?: string | null };
...
export function useExternalClients() {
  ...
  /* `destino`: "aprovar" e o portal completo; "materiais" e a pagina so de
     pedidos de material (mesmo token, rota diferente). */
  const copyLink = async (clientId: string, period?: { start: string; end: string } | null, destino: "aprovar" | "materiais" = "aprovar"): Promise<string | undefined> => {
    ...
    const url = `${PORTAL_ORIGIN}/${destino}/${token}`;
    try {
      await navigator.clipboard.writeText(url);
      toast.success(destino === "materiais" ? "Link de pedidos copiado!" : period ? "Link do período copiado!" : "Link de aprovação copiado!");
    }
