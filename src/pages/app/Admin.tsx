...
function StoryBankAdminCard() {
  const { data: trends = [] } = useStoryTrends();
  const refresh = useRefreshStoryTrends();
  const last = trends[0]?.created_at ? new Date(trends[0].created_at) : null;
  return (
    <div className="rounded-2xl border border-border bg-card p-4 mb-6 flex items-center justify-between gap-3 flex-wrap">
      <div className="flex items-center gap-3">
        <span className="w-9 h-9 rounded-xl bg-primary/10 grid place-items-center shrink-0"><Clapperboard className="h-4 w-4 text-primary" /></span>
        <div>
