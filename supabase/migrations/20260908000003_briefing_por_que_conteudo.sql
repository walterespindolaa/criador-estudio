-- ═══════════════════════════════════════════════════════════════════════════
-- BRIEFING: A ETAPA "POR QUE CONTEÚDO" (pedido da Gabi, 08/09/2026)
--
-- O formulário abria com CNPJ e endereço. Agora abre com as quatro perguntas
-- que o cliente responde com vontade (por que quer conteúdo, o que espera
-- conquistar, como quer ser percebido, que tema o trava). Duas delas usam
-- chaves que já existiam (mainGoal, perception6m) e duas são novas:
--   whyContent   -> por que quer começar ou fortalecer o conteúdo
--   blockedTopic -> tema que ele quer abordar e não sabe como
--
-- Sem isso no apply_intake, as duas novas chegariam do formulário e seriam
-- jogadas fora na hora de aplicar na ficha.
-- Rodar DEPOIS de 20260908000002_intake_para_cliente_com_cria.
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  foreach _txt in array array[
    ...
    -- Novas da etapa "Por que conteúdo".
    'whyContent','blockedTopic'
  ] loop
  ...
  if _dono is not null then
    -- "whyContent" responde exatamente a pergunta "por que você cria conteúdo"
    -- do Brandbook do criador, então vai direto pra lá.
    _mapa := array[
      ...
      array['whyContent',    'moodboard-contexto',    'por-que'],
