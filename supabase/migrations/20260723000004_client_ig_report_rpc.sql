-- Relatório white-label do cliente: uma RPC que devolve, num único jsonb, as
-- mídias do período + a demografia de audiência (snapshot atual) + os stories do
-- período. Antes o relatório só tinha get_client_ig_media (mídias), então
-- demografia e stories ficavam de fora. Mesma checagem de segurança do
-- get_client_ig_media: só o gestor dono daquele cliente lê, e a fonte é a conta
-- CRIA do próprio cliente (cria_owner_id, com crm_client_id null = conta própria).
...
begin
  ...
  -- Mídias do período (mesmos campos do get_client_ig_media, conta própria = crm_client_id null)
